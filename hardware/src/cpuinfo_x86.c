...
static X86Info kEmptyX86Info = { 0 };
static CacheInfo kEmptyCacheInfo = { 0 };
...
X86Info* GetX86Info(void) {
  ...
  if (0 == init)
  {
      ...
      SetVendor(leaf_0, kEmptyX86Info.vendor);
      if (is_intel || is_amd) {
          const uint32_t max_cpuid_leaf = leaf_0.eax;
          const OsSupport os_support = CheckOsSupport(max_cpuid_leaf);
          ParseCpuId(max_cpuid_leaf, os_support, &kEmptyX86Info);
          if (is_amd) {
              ParseExtraAMDCpuId(&kEmptyX86Info, os_support);
          }
      }
...
CacheInfo* GetX86CacheInfo(void) {
  ...
  if (IsVendor(leaf_0, "GenuineIntel")) {
    ParseLeaf2(max_cpuid_leaf, &kEmptyCacheInfo);
    ParseLeaf4(max_cpuid_leaf, &kEmptyCacheInfo);
  }
  return &kEmptyCacheInfo;
}
