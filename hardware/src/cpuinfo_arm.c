...
static ArmInfo kEmptyArmInfo = {0};

static ProcCpuInfoData kEmptyProcCpuInfoData;
...
ArmInfo *GetArmInfo(void) {
  ...
  // /proc/cpuinfo).  
    ...
    if (0 == init)
    {
        FillProcCpuInfoData(&kEmptyArmInfo, &kEmptyProcCpuInfoData);
        const HardwareCapabilities hwcaps = CpuFeatures_GetHardwareCapabilities();
        for (size_t i = 0; i < ARM_LAST_; ++i) {
            if (CpuFeatures_IsHwCapsSet(kHardwareCapabilities[i], hwcaps)) {
                kSetters[i](&kEmptyArmInfo.features, true);
            }
        }

        FixErrors(&kEmptyArmInfo, &kEmptyProcCpuInfoData);
        
        init = 1;
    }
  return &kEmptyArmInfo;
}
