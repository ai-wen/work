...
SSL_CTX_set_verify----配置启用双向认证 | SSL_VERIFY_PEER| SSL_VERIFY_FAIL_IF_NO_PEER_CERT
...
void gen_server() {
    ...
    */
    ...
    //SSLv3_server_method();
    SSL_CTX  *ctx = SSL_CTX_new(SSLv23_server_method());//SSLv23_client_method
    
    /* use only TLSv1.2 protocol */
    SSL_CTX_set_options(ctx, SSL_OP_NO_TLSv1 | SSL_OP_NO_TLSv1_1 | SSL_OP_NO_SSLv3);  
   ...
   // SSL_VERIFY_FAIL_IF_NO_PEER_CERT---要求客户端需要提供证书，但验证发现单独使用没有证书也会放行
    ...
    /* 载入服务端的数字证书， 此证书用来发送给客户端。 证书里包含有公钥 */
                ...
                while (1) {
                    ...
                    if ((new_fd = accept(sockfd, (struct sockaddr*)&their_addr, &len)) != -1) {
                       ...
                       /* 将连接用户的 socket 加入到 SSL */
                        ...
                        unsigned char buf[256] = "server->client";
                        len = SSL_write(ssl, buf, sizeof(buf));
